...
- **BYOK is the only AI auth path.** No platform-level `OPENAI_API_KEY` env var. Users add their own key at `/dashboard/settings`, stored in `profiles.openai_api_key`. The crawl route fetches it via `getUserOpenAIKey(user.id)` and passes it explicitly to `embedTexts(apiKey, ...)`. If you ever feel tempted to fall back to an env var "for development," don't — the codebase is intentionally inconsistent without one so missing keys fail loud.
